net.Receive("SCPSOUNDVF:SQL:PLAY_SCHEDULED_SOUND", function()
    local title = net.ReadString()
    local soundPath

    for _, category in pairs(SCPSNDVF.CONFIG.BUTTONS) do
        for _, cmd in ipairs(category.commands) do
            if cmd.title == title then
                soundPath = cmd.sound
                break
            end
        end
    ...
    if soundPath then
        ...
        sound.PlayURL("https://up-rp.4kaptured.com/docs/"..soundPath, "noblock", function(soundChannel)
